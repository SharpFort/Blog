...
> **摘要**：本文记录了为 Monolith 边缘博客新增「友链」功能模块的完整过程，包括后端数据表设计、适配器模式下的多端同步、前端页面开发与路由注册。重点梳理了开发中踩过的 8 个坑——PowerShell UTF-8 编码破坏、模板字面量转义、注释锚点定位失败、忘记更新 schema 基线校验、接口修改后适配器遗漏、git checkout 文件重置等——并总结了一套可复用的 9 步开发工作流，适用于 Monolith 后续任何新功能模块的添加。
